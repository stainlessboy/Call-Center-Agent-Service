...
- Гибридный режим бот/оператор: передача диалога живому оператору через Asaka chat-middleware (Socket.IO)
- Квалификационный опросник перед показом продуктов (qualify_flow)
- Поиск офисов (филиалы / ЦБУ / точки продаж)
- Захват лидов: имя + телефон → БД
- Админ-панель SQLAdmin на `/admin` (включая загрузку seed-данных через `/admin/seed`)
...
```
START → router
          ├─► faq           LLM + 11 инструментов
          ├─► calc_flow     Калькулятор кредита/вклада + захват лида
          ├─► qualify_flow  Квалификационный опросник (детерминированный)
...
| Кнопка расчёта + `flow == "product_detail"` | → `calc_flow` (без LLM) |
| Всё остальное | → `faq` |
...
class BotState(TypedDict):
    ...
    lang: str                     # "ru" | "en" | "uz" — определяется детектором языка
    _route: str                   # Результат роутера
    session_id: str               # UUID сессии
    user_id: int                  # Telegram user ID
```

**dialog dict** (`_default_dialog()` в `app/agent/state.py`):

```python
{
    "flow": None,              # None | "show_products" | "product_detail" | "calc_flow" | "qualify_flow"
    ...
    "qualify_category": None,  # Категория опросника qualify_flow
    "qualify_node": None,      # Текущий узел дерева вопросов
    "qualify_answers": {},     # Накопленные ответы (фильтры для БД)
    "fallback_streak": 0,      # Счётчик fallback подряд (3 → кнопка оператора)
    "last_lang": "ru",         # Последний язык ответа
    "offices": [],             # Найденные офисы (find_office)
    "selected_office": None,   # Выбранный офис
    "office_type": None,       # filial | sales_office | sales_point
}
```

«Липкие» флаги сессии (например `lang_switch_offered`) переживают сброс диалога: при сбросе в середине сессии используется `_reset_dialog()` вместо голого `_default_dialog()`, а `_finalize_turn()` дополнительно переносит их из предыдущего dialog.
...
### 11 инструментов (`app/agent/tools.py`, список `_FAQ_TOOLS`)

| # | Tool | Когда LLM вызывает | Параметры | Влияние на dialog |
|---|------|--------------------|-----------|--------------------|
| 1 | `find_office(office_type, query)` | адрес / филиал / офис | `office_type, query` | `offices=[...]`, список офисов |
| 2 | `select_office(office_name)` | Выбор офиса из списка | `office_name: str` | `selected_office`, карточка офиса |
| 3 | `get_office_types_info()` | «чем филиал отличается от ЦБУ» | — | Без изменений |
| 4 | `get_currency_info()` | курс / доллар / exchange rate | — | Без изменений |
| 5 | `show_credit_menu()` | "хочу кредит" (без типа) | — | Подменю кредитов |
| 6 | `get_products(category)` | Запрос конкретного типа продукта | `category: str` | `flow="show_products"` (или запуск qualify_flow), загрузка из БД |
| 7 | `select_product(product_name)` | Выбор продукта из списка | `product_name: str` | `flow="product_detail"`, карточка; при ненайденном продукте flow не меняется |
| 8 | `start_calculator()` | ✅ рассчитать / calculate | — | `flow="calc_flow"`, первый вопрос |
| 9 | `custom_loan_calculator(amount, term, rate)` | Аннуитет по своим цифрам (без продукта) | `amount, term, rate` | Без изменений |
| 10 | `faq_lookup(query)` | Любой вопрос о банке | `query: str` | Гибридный FAQ-поиск (лексический + семантический, три уровня уверенности) |
| 11 | `request_operator(reason)` | "оператор" / операции с идентификацией | `reason: str` | Сигнал подключения оператора |

`clarify` определён в `tools.py`, но временно исключён из `_FAQ_TOOLS` (закомментирован).
...
2. Оператор отвечает через **Asaka chat-middleware**: ответ приходит в Socket.IO-колбэк `_on_agent_message` ([app/api/fastapi_app.py](app/api/fastapi_app.py)).
3. Колбэк зовёт `agent_client.resume_human_mode(session_id, text)` → `graph.ainvoke(Command(resume=text), config)`. Граф возобновляется ровно с точки `interrupt`, `operator_reply` становится `answer`.
...
- Если `faq_lookup` не нашёл уверенного ответа → `fallback_streak += 1`.
...
| `CreditProductOffer` | `credit_product_offers` | Кредитные продукты (одна строка на продукт) |
...
│   │   ├── graph.py            # Определение графа (5 нод)
│   │   ├── agent.py            # Класс Agent (lifecycle, invocation)
│   │   ├── state.py            # BotState + _default_dialog() / _reset_dialog()
│   │   ├── tools.py            # 11 LangChain-инструментов (_FAQ_TOOLS)
│   │   ├── products.py         # Загрузка и форматирование продуктов
│   │   ├── branches.py         # Поиск офисов + карточки (филиал/ЦБУ/точка продаж)
│   │   ├── qualify.py          # Деревья квалификационного опросника
│   │   ├── rate_rules.py       # Движок динамических ставок (CreditRateRule)
│   │   ├── calc_extractor.py   # LLM-экстракторы значений калькулятора
│   │   ├── lang_detect.py      # LLM-детектор языка (ru/en/uz)
│   │   ├── lang_heuristic.py   # Быстрые эвристики языка
│   │   ├── pii_masker.py       # Маскирование PII перед отправкой в OpenAI
│   │   ├── i18n.py             # Переводы (ru/en/uz)
│   │   ├── intent.py           # Определение намерения (категория, триггеры)
│   │   ├── constants.py        # Константы, context vars
│   │   ├── llm.py              # Инициализация LLM (ChatOpenAI)
│   │   ├── checkpointer.py     # Postgres/Memory checkpointer
│   │   └── nodes/
│   │       ├── router.py       # Роутер (human_mode → calc_flow → qualify_flow → faq)
│   │       ├── faq.py          # FAQ нода (LLM + tools, 3 раунда)
│   │       ├── calc_flow.py    # Калькулятор + lead capture
│   │       ├── qualify_flow.py # Квалификационный опросник
│   │       ├── human_mode.py   # Оператор (interrupt/resume)
│   │       └── helpers.py      # _finalize_turn, обрезка истории
│   ├── api/
│   │   └── fastapi_app.py      # FastAPI: webhook, /health, middleware-колбэки, inactivity watcher
│   ├── bot/
│   │   ├── handlers/
│   │   │   └── commands.py     # Все Telegram-хэндлеры
│   │   ├── keyboards/          # Билдеры клавиатур
│   │   ├── middlewares/        # aiogram middlewares (chat_service, rate limit)
│   │   └── i18n.py             # Переводы интерфейса бота
│   ├── db/
│   │   ├── models.py           # SQLAlchemy ORM
│   │   ├── session.py          # AsyncSession factory
│   │   ├── events.py           # События SQLAlchemy (пересчёт FAQ-эмбеддингов)
│   │   └── alembic/            # Миграции
│   ├── services/
│   │   ├── chat_service.py     # Сессии, сообщения, timeout, hybrid mode
│   │   ├── agent_client.py     # Обёртка над Agent
│   │   ├── chat_middleware_client.py  # Asaka chat-middleware (JWT + Socket.IO)
│   │   ├── middleware_registry.py     # Process-wide доступ к ChatMiddlewareClient
│   │   ├── middleware_files.py # MinIO upload + пересылка медиа
│   │   └── telegram_sender.py  # Отправка сообщений в Telegram
│   ├── admin/
│   │   ├── views.py            # SQLAdmin ModelView
│   │   ├── auth.py             # Авторизация (env-based)
│   │   ├── setup.py            # Инициализация SQLAdmin
│   │   ├── dashboard_view.py   # Дашборд админки
...
| `WEBHOOK_SECRET` | prod | — | Секрет для webhook (сравнение timing-safe) |
| `FORWARDED_ALLOW_IPS` | — | `127.0.0.1` | Прокси, которым доверяем `X-Forwarded-*` |
...
1. Пользователь жмёт inline-кнопку «Живой оператор» (callback `human:<session_id>`) — бот предлагает её сам после 3 fallback подряд или по `request_operator`
2. `ChatSession.human_mode = True`, бот открывает чат в Asaka chat-middleware (`start_chat`)
3. Сообщения пользователя сохраняются в БД и пересылаются в middleware (LLM их не обрабатывает); медиа уходит через MinIO
4. Ответы оператора приходят по Socket.IO (`_on_agent_message`) → пересылаются в Telegram и резюмируют граф
5. Кнопка «Бот» (callback `bot:<session_id>`) или фоновый watcher (60 сек, таймаут оператора) возвращают сессию боту

REST API для операторов нет — весь обмен идёт через chat-middleware (протокол: [docs/CHAT_MIDDLEWARE_INTEGRATION.md](docs/CHAT_MIDDLEWARE_INTEGRATION.md)).
...
**Шаг 3.** Если инструмент меняет dialog state → добавить обработчик в `app/agent/nodes/faq.py` функция `_update_dialog_from_tools()`:
...
python3 -m pytest tests/ -v -k "faq"  # только FAQ тесты
```
